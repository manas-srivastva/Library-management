...
const nav = [
  ...
  { to: '/app/copies', label: 'Book Copies', icon: BookCopy },
  ...
  return (
    <>
              ...
              <>
                ...
                <span
                  className={cn(
                    ...
                    collapsed && 'left-1/2 -translate-x-1/2 top-0 h-full w-0.5',
                  )}
                />
                <item.icon
                  className={cn(
                    'h-[18px] w-[18px] shrink-0 transition-colors',
                    isActive ? 'text-brand-400' : 'text-fg-subtle group-hover:text-fg',
                  )}
                />
      ...
      <div className="mt-6 space-y-0.5">
        <p className={cn('px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-fg-faint', collapsed && 'sr-only')}>
          Account
        </p>
        {secondary.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                ...
                isActive ? 'bg-bg-elevated text-fg' : 'text-fg-muted hover:bg-bg-elevated/60 hover:text-fg',
                collapsed && 'justify-center px-0',
              )
            }
            title={collapsed ? item.label : undefined}
...
function LogoBlock({ collapsed }: { collapsed: boolean }) {
  return <Logo size={36} showText={!collapsed} subtitle={collapsed ? undefined : 'Smart Library'} />;
}
            ...
            <div className="relative overflow-hidden rounded-xl border border-brand-500/20 bg-gradient-to-br from-brand-500/10 to-accent-500/5 p-3.5">
              ...
              <div className="relative">
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-xs font-semibold text-fg">AI Assistant</p>
                </div>
